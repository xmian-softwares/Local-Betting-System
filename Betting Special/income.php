...
$tot_inc = 0;
$tot_prof = 0;
$sel_count = $con->query("SELECT * FROM user WHERE `Access` = '1'");
...
<input type="text" name="admin" required  placeholder="Enter Admin Name" hidden value="<?php echo $user?>">
<input type="password" name="pwd" required placeholder="Enter Admin Password"hidden value="<?php echo $pwd?>">
<input type="submit" value="Income" style="margin-top:10px;width:100%;color:white;background:Black">
</form></center>
...
<?php if($co == 0 && $sel->num_rows > 0):?>
  <div style="box-shadow: 4px 4px 4px 4px gray;background:linear-gradient(to right,black,red);color:Red" hidden>
...
<?php if($co == 1 && $er == 0 && $sel->num_rows > 0):?>
  <div style="box-shadow: 4px 4px 4px 4px gray;background:linear-gradient(to right,black,green);color:Green" hidden>
    <center><h3 style="padding:3px;background:White;">User: <?php echo $usr?></h3></center>
    <br><center>
    <?php $cot = 0;
      while($cot < count($res)):?>
    ...
    <?php endwhile;?>
    <?php  $sel_counts = $con->query("SELECT * FROM user WHERE `Id`='$usr' AND `Access` = '1'");
    if($sel_counts->num_rows){
      $sel_odd = $con->query("SELECT * FROM confirm WHERE us_id ='$usr'"); 
      $sel_amt = $con->query("SELECT * FROM user WHERE Id = '$usr'");
      $row_amt = $sel_amt->fetch_array();
      while($row_odd = $sel_odd->fetch_array()){
          $odd = $odd*$row_odd['Odd'];
      }
      $tot_prof = $tot_prof-(($row_amt['Amount']*0.90)*$odd);
    }?>
...
<div style="box-shadow: 4px 4px 4px 4px gray;background:linear-gradient(to right,black,blue);color:Blue" hidden> 
    <center><h3 style="padding:3px;background:White;">User: <?php echo $usr?></h3></center>
    <br><center>
    <?php $cot = 0;
      while($cot < count($res)):?>
...
<br><div>
 <?php while($row_count = $sel_count->fetch_array()){
   $tot_inc = $tot_inc+$row_count['Amount'];
   $tot_prof = $tot_prof+$row_count['Amount'];
 }?>
 
 <center><div style="box-shadow:4px 4px 4px 4px gray;">
 <p style="background:blue;padding:10px;color:white">Total Income</p>
 <h4><?php echo intval($tot_inc);?> Birr</h4>
 </div><br>
 
 <div style="box-shadow:4px 4px 4px 4px gray;">
 <p style="background:Red;padding:10px;color:white">Total Profit</p>
 <h4><?php echo intval($tot_prof);?> Birr</h4>
 </div></center>
