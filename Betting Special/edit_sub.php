...
$sel = $con->query("SELECT * FROM sub_admins WHERE `User` = '$usr' AND `Password` = '$pass'");
$row = $sel->fetch_array();
...
<form method='post' action="edit_sub_fin.php"><br>
 <input type="text" name="admin" required  placeholder="Enter Admin Name" hidden value="<?php echo $user?>">
 <input type="password" name="pwd" required placeholder="Enter Admin Password"hidden value="<?php echo $pwd?>">
 <input type="text" name="OAU" value="<?php echo $row['User']?>" hidden>
 <input type="password" name="OAP" value="<?php echo $row['Password']?>" hidden>
 
 Sub Admin Name<br><input type="text" name="NAU" value="<?php echo $row['User']?>"><br><br>
